package com.merise.net.heart.view.gesture;
...
public class GestureLockViewGroup extends RelativeLayout {
    ...
    private List<Integer> firstChoose = new ArrayList<>();
    private List<Integer> secondChoose = new ArrayList<>();
    ...
    private boolean isSettingPwd;//是否为设置手势密码

    public boolean isSettingPwd() {
        return isSettingPwd;
    }

    public void setSettingPwd(boolean settingPwd) {
        isSettingPwd = settingPwd;
    }
    ...
    public static final int COMFIRM = 0x02;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                GestureLockView firstView = mGestureLockViews[0];
                GestureLockView lastView = mGestureLockViews[mGestureLockViews.length - 1];
                if (x < firstView.getLeft() || x > lastView.getRight() || y < firstView.getTop() || y > lastView.getBottom()) {
                    return false;
                }
                TLog.log(TAG, "ACTION_DOWN");
                if (!isSettingPwd && mTryTimes == 0) {
                    reset();
                    invalidate();
                    return false;
                }
            ...
            case MotionEvent.ACTION_UP:
                TLog.log(TAG, "ACTION_UP");
                mPaint.setColor(mFingerUpColor);
                mPaint.setAlpha(50);
                if (mOnGestureLockViewListener != null && mChoose.size() < minLength && mChoose.size() > 0) {
                    mOnGestureLockViewListener.onGestureIllegal();
                } else {
                    if (isSettingPwd && mChoose.size() > 0) {
                        if (firstChoose.size() > 0) {
                            secondChoose.addAll(mChoose);
                            TLog.log(TAG, "firstChoose1:" + firstChoose.toString());
                            TLog.log(TAG, "secondChoose1:" + secondChoose.toString());
                        }
                        if (firstChoose.size() == 0) {
                            firstChoose.addAll(mChoose);
//                            if(firstChoose==mAnswer){}
                            if (mOnGestureLockViewListener != null) {
                                mOnGestureLockViewListener.isComfirmGesture(COMFIRM);//第一次输入密码之后，提醒用户确认手势密码
                            }
                            TLog.log(TAG, "firstChoose2:" + firstChoose.toString());
                            TLog.log(TAG, "secondChoose2:" + secondChoose.toString());
                        }
                        if (secondChoose.size() >= minLength && firstChoose.size() >= minLength) {
                            isSame = checkGesturePwd();
                            mOnGestureLockViewListener.onSettingGesture(isSame);
                            clearData();
                        }
                    } else if (!isSettingPwd) {
                        if (mOnGestureLockViewListener != null && mChoose.size() >= minLength) {
                            this.mTryTimes--;
     ...
     * 检测设置手势密码是否成功
     */
    private boolean checkGesturePwd() {
        ...
        mAnswer = new int[firstChoose.size()];
        for (int i = 0; i < firstChoose.size(); i++) {
            mAnswer[i] = firstChoose.get(i);
        }
        return true;
    }
    ...
    public interface OnGestureLockViewListener {
         ...
         * 手势非法
         */
        void onGestureIllegal();

        /**
         * 设置手势密码回调
         */
        void onSettingGesture(boolean result);
         ...
         * @param times
         */
        void isComfirmGesture(int times);
    }
